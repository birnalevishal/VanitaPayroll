...
{
    public partial class BankLetterVanita : System.Web.UI.Page
    {
        ...
        protected void btnShow_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ReportDataSource datasource = new ReportDataSource("dsBankLetterVanita", dv.ToTable());
                ...
                ReportViewer1.LocalReport.ReportPath = Server.MapPath("~/Reports/rptBankLetterVanita.rdlc");

                //----------Getting Address and period-----------------------------------------
                string strqry = "SELECT Address FROM dbo.M_Bank WHERE BankCd = " + ddlBankName.SelectedValue;
                DataTable dtBank = SqlHelper.ExecuteDataTable(strqry, AppGlobal.strConnString);
                string add = dtBank.Rows[0]["Address"].ToString();

                string MonthFrom = "1" + "-"+ ddlMon.SelectedItem + "-" + ddlYear.SelectedValue;
                string MonthTo = Days + "-" + ddlMon.SelectedItem + "-" + ddlYear.SelectedValue;
                //------------------------------------------------------------------------------

                ReportParameter p1 = new ReportParameter("BankName", ddlBankName.SelectedItem.ToString());
                ReportParameter p2 = new ReportParameter("AsOnDAte", DateTime.Now.ToString("dd/MMM/yyyy"));
                ReportParameter p3 = new ReportParameter("BranchName", ddlBranchName.SelectedItem.ToString());
                ReportParameter p4 = new ReportParameter("Address", add);
                ReportParameter p5 = new ReportParameter("MonthFrom", MonthFrom);
                ReportParameter p6 = new ReportParameter("MonthTo", MonthTo);

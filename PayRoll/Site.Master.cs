...
namespace PayRoll
{
    public partial class SiteMaster : MasterPage
    {
        ...
        private void CreateReportsMenus(string parentMenuId = "", string menuItem = "")
        {
            ...
            if (RoleId == "1")
            {
                strQry = "select * from M_Menu where  IsActive='Y' AND ParentMenuId IN(4,11,98,124) order by Title";
            }
            else
            {
                string MMIds = Session["MMIds"].ToString();
                strQry = "select * from M_Menu where IsActive='Y' AND ParentMenuId IN(4,11,98,124) AND MenuId IN(" + MMIds + ") order by Title";
            }
        ...
        private void CreateMasterMenus(string parentMenuId = "", string menuItem = "")
        {
            ...
            if (RoleId == "1")
            {
                strQry = "select * from M_Menu where  IsActive='Y' AND ParentMenuId IN(2,9,3,5,6,123) order by Title";
            }
            else
            {
                string MMIds = Session["MMIds"].ToString();
                strQry = "select * from M_Menu where IsActive='Y' AND ParentMenuId IN(2,9,3,5,6,123) AND MenuId IN(" + MMIds + ") order by Title";
            }
